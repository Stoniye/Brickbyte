...
impl Brickbyte {
    ...
    fn init_gl(
        ...
        let egui_painter = egui_glow::Painter::new(gl_arc.clone(), "", None, true)
            ...
            event_loop
                .create_window(
                    Window::default_attributes()
                        .with_title("Brickbyte")
                        .with_inner_size(PhysicalSize::new(1200.0, 800.0)),
                )
                .unwrap(),
        );

        let (gl_surface, gl_context, gl, egui_context, egui_painter, egui_state) =
            self.init_gl(&window);
    ...
    fn window_event(
        ...
        match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                if state == ElementState::Pressed && key_code == KeyCode::Escape {
                    self.gamestate.as_ref().expect("Without gamestate, the game has to crash!").release_cursor();
                }

                self.gamestate
                    .as_mut()
                    .unwrap()
                    .keyboard_input(state, key_code);
            }
            ...
            WindowEvent::MouseInput { state, button, .. } => {
                self.gamestate
                    .as_mut()
                    .unwrap()
                    .mouse_button_input(state, button);
            }
    ...
    fn device_event(
        ...
        if let DeviceEvent::MouseMotion { delta } = event {
            self.gamestate
                .as_mut()
                .unwrap()
                .mouse_motion_input(delta, event);
        }
    }
