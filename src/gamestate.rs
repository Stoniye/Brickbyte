...
impl GameState {
    pub fn new(
        ...
        window: Arc<Window>,
        egui_context: egui::Context,
        mut egui_painter: egui_glow::Painter,
        egui_state: State,
    ) -> Self {
        let (block_texture, egui_block_atlas_is, egui_ui_atlas_id) =
            Self::load_textures(&gl, &mut egui_painter);
    ...
    pub fn generate_world(&mut self) {
        let nm = NoiseMap::new(PerlinNoise::new())
            .set_seed(Seed::of("12345678910"))
            .set_size(Size::of(CHUNK_DIMENSION as i64, CHUNK_DIMENSION as i64))
            .set_step(Step::of(0.0005, 0.0005));
        ...
        for x in -X_CHUNKS..X_CHUNKS {
            for y in -X_CHUNKS..Y_CHUNKS {
                self.world.insert_chunk(
                    ...
                    self.program.unwrap(),
                    nm.generate_chunk(x as i64, y as i64),
                );
            }
        }
    ...
    ) -> (NativeTexture, TextureId, TextureId) {
        unsafe {
            //Block
            let block_texture: NativeTexture =
                gl.create_texture().expect("Failed to create texture var");
            ...
            for block_pixel in block_buffer.chunks_exact(4) {
                ...
                block_floats.extend_from_slice(&[r * a, g * a, b * a, a]);
            }

            let block_bytes: &[u8] = {
                std::slice::from_raw_parts(
                    block_floats.as_ptr() as *const u8,
                    block_floats.len() * size_of::<f32>(),
                )
            };
            ...
            gl.tex_image_2d(
                ...
                glow::RGBA,
                glow::FLOAT,
                glow::PixelUnpackData::Slice(Some(block_bytes)),
            );
            ...
            for ui_pixel in ui_buffer.chunks_exact(4) {
                ...
                ui_floats.extend_from_slice(&[r * a, g * a, b * a, a]);
            }

            let ui_bytes: &[u8] = {
                std::slice::from_raw_parts(
                    ui_floats.as_ptr() as *const u8,
                    ui_floats.len() * size_of::<f32>(),
                )
            };
            ...
            gl.tex_image_2d(
                ...
                glow::RGBA,
                glow::FLOAT,
                glow::PixelUnpackData::Slice(Some(ui_bytes)),
            );
            ...
            return (
                block_texture,
                egui_painter.register_native_texture(block_texture),
                egui_painter.register_native_texture(ui_texture),
            );
        };
    }
    ...
    pub fn new_frame(&mut self, delta_time: f32) {
        self.player
            .update_pos(delta_time, self.keys_pressed.clone(), &self.world)
    }
    ...
    fn render_game(&mut self) {
        ...
        let view = Mat4::look_at_rh(
            self.player.get_head_pos(),
            self.player.get_head_pos() + self.player.get_camera_front(),
            Vec3::Y,
        );
        ...
        unsafe {
            ...
            self.world
                .render_world(&self.gl, pv, Some(self.block_texture));
        }

        let raw_input = self.egui_state.take_egui_input(&self.window);
        self.egui_context.begin_pass(raw_input);

        let painter_layer = self.egui_context.layer_painter(egui::LayerId::new(
        ...
        painter_layer.rect(
            egui::Rect::from_two_pos(
                center + egui::vec2(-HOTBAR_WIDTH / 2.0, center.y - HOTBAR_HEIGHT),
                center + egui::vec2(HOTBAR_WIDTH / 2.0, center.y - HOTBAR_POS_HEIGHT),
            ),
            2.0,
            Color32::from_black_alpha(100),
            Stroke::new(LINE_THICKNESS, Color32::GRAY),
            ...
            painter_layer.line_segment(
                [
                    center
                        + egui::vec2(
                            -((HOTBAR_WIDTH / 2.0)
                                - (HOTBAR_SLOT_WIDTH + (LINE_THICKNESS / 2) as f32))
                                + (i as f32 * HOTBAR_SLOT_WIDTH),
                            center.y - HOTBAR_HEIGHT,
                        ),
                    center
                        + egui::vec2(
                            -((HOTBAR_WIDTH / 2.0)
        ...
        painter_layer.rect(
            egui::Rect::from_two_pos(
                center
                    + egui::vec2(
                        -HOTBAR_WIDTH / 2.0
                            + (self.selected_hotbar_slot_index as f32 * HOTBAR_SLOT_WIDTH),
                        center.y - HOTBAR_HEIGHT,
                    ),
                center
                    + egui::vec2(
                        -((HOTBAR_WIDTH / 2.0) - (HOTBAR_SLOT_WIDTH + LINE_THICKNESS as f32))
                            + (self.selected_hotbar_slot_index as f32 * HOTBAR_SLOT_WIDTH),
                        center.y - HOTBAR_POS_HEIGHT,
                    ),
            ),
            2.0,
            Color32::TRANSPARENT,
            Stroke::new(LINE_THICKNESS, Color32::WHITE),
            egui::StrokeKind::Inside,
        );
        ...
        for i in 0..5 {
            ...
            let uv = egui::Rect::from_min_max(
                egui::pos2(
                    (i as f32 * 16.0 + 0.1) / TEX_SIZE as f32,
                    0.0 + PIXEL_MARGIN as f32,
                ),
                egui::pos2(
                    ((i + 1) as f32 * 16.0 - 0.1) / TEX_SIZE as f32,
                    0.0625 - PIXEL_MARGIN as f32,
                ),
            );
        ...
        for i in 0..self.player.get_health() {
            ...
            let uv = egui::Rect::from_min_max(egui::pos2(0.0, 0.0), egui::pos2(0.0625, 0.0625));
    ...
    pub fn mouse_button_input(&mut self, state: ElementState, button: MouseButton) {
        if state == ElementState::Pressed {
            ...
            let ndc = Vec4::new(
                (2.0 * (size.width as f32 / 2.0)) / size.width as f32 - 1.0,
                1.0 - (2.0 * (size.height as f32 / 2.0)) / size.height as f32,
                -1.0,
                1.0,
            );
            ...
            let hit = { self.world.raycast_block(ray_origin, ray_dir, 10.0) };
            ...
            if let Some(hit) = hit {
                match button {
                    ...
                    MouseButton::Right => {
                        ...
                        let player_pos: IVec3 = IVec3::new(
                            self.player.get_pos().x.floor() as i32,
                            self.player.get_pos().y.ceil() as i32,
                            self.player.get_pos().z.floor() as i32,
                        );
                        let player_head_pos: IVec3 = IVec3::new(
                            self.player.get_head_pos().x.floor() as i32,
                            self.player.get_head_pos().y.floor() as i32,
                            self.player.get_head_pos().z.floor() as i32,
                        );

                        if block_pos != player_pos && block_pos != player_head_pos {
                            self.world.set_block(
                                hit.prev_block_pos,
                                self.selected_hotbar_slot_index + 1,
                                &self.gl.clone(),
                            );
                        }
                    }
